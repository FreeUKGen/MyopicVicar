...
def self.check_for_replace(filename,userid,digest)
 ...
 def self.process(type,range)
  ...
  userids = range.split("/")
    ...
    @@message_file.puts  "Started an analysis of problem email addresses"
    number = 0
 ...
 header[:disabled] = header[:disabled].to_i
   ...
   userid.save
    ...
    if userid.errors.any?
      ...
      @@message_file.puts "#{header[:userid]};#{userid.errors.messages};#{header[:email_address]}; #{previous_userid}"
      p "#{header[:userid]};#{userid.errors.messages};#{header[:email_address]}; #{previous_userid}"
