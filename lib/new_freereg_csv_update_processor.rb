...
class CsvFile < CsvFiles
	...
	attr_accessor :header, :list_of_registers, :header_error, :system_error, :data_hold, 
	  ...
	  :all_existing_records, :total_files, :total_records, :total_data_errors, :total_header_errors
	...
	def clean_up_message(project)
	    File.delete(project.member_message_file)
	end
	...
	def communicate_failure_to_member(project,message)
	    ...
	    UserMailer.batch_processing_failure(file,@userid,@file_name).deliver
	...
	def communicate_file_processing_results(project)
		project.member_message_file.close
	    file = project.member_message_file
		 UserMailer.batch_processing_success(file,@header[:userid],@header[:file_name]).deliver 
	    return true
	end
