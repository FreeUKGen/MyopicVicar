class MissingPlaceFields
  ...
  Place.no_timeout.each do |place|
    ...
    if place.genuki_url.blank? && place.not_disabled
      errors = errors + 1
       output_file.puts ("#{place.id}, #{place.chapman_code}, #{place.place_name}, no genuki"  )
    end
    if place.county.blank? && place.not_disabled
      errors = errors + 1
    ...
    if place.chapman_code.blank? && place.not_disabled
      errors = errors + 1
       output_file.puts ("#{place.id}, #{place.chapman_code}, #{place.place_name}, no chapman code"  )
    end
    if (place.latitude.nil? && place.longitude.nil?) && place.not_disabled 
      errors = errors + 1
