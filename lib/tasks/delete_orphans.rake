task :delete_orphans,[:limit] => [:environment] do |t, args|
  Mongoid.load!("#{Rails.root}/config/mongoid.yml")
  file_for_warning_messages = "#{Rails.root}/log/orphan_files_processing.txt"
  FileUtils.mkdir_p(File.dirname(file_for_warning_messages))
  output_file = File.new(file_for_warning_messages, "a")
  file = File.join("#{Rails.root}/log/orphan_files_list.txt")
  ...
  input_file.each_line  do |line|
    ...
    record_number = 0
    if  Freereg1CsvEntry.where(:freereg1_csv_file_id => line).exists?
