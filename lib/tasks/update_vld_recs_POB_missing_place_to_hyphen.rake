desc "Update VLD Records, and Individual recs and Search recs birth place missing to hyphen"
task :update_vld_recs_POB_missing_place_to_hyphen, [:limit, :fix, :email, :restriction] => :environment do |t, args|
  ...
  def self.update_search_record(year, rec, fix, listing)
    rec.set(birth_place: '-') if fix
    write_csv_line(listing, year, 'SearchRecord', rec._id, rec.chapman_code, 'Birth_place update null -> hyphen')
  end
  ...
  file_for_log = "log/update_vld_recs_POB_missing_place_to_hyphen_#{start_time.strftime('%Y%m%d%H%M')}.log"
  file_for_listing = "log/update_vld_recs_POB_missing_place_to_hyphen_#{start_time.strftime('%Y%m%d%H%M')}.csv"
  ...
  one_county = false
  single_county = 'XXX'
  initial_message = "Started Update of VLD Records and associated individual/search records - missing place to hyphen with fix = #{fixit} - search record limit = #{record_limit}"
  ...
  if args_valid == true
    ...
    message = "Finished Update of VLD Records and associated individual/search records - missing place to hyphen - run time = #{run_time}"
    output_to_log(file_for_log, message)
    message = "Processed #{search_recs_processed} Search Records records - see log/update_vld_recs_POB_missing_place_to_hyphen_YYYYMMDDHHMM.csv and .log for output"
    output_to_log(file_for_log, message)
    ...
    unless args.email == 'N'
      ...
      email_body = "Processed #{search_recs_processed} records - update_vld_recs_POB_missing_place_to_hyphen csv output file attached"
      report_name = "update_vld_recs_POB_missing_place_to_hyphen_#{start_time.strftime('%Y%m%d%H%M')}.csv"
