...
task :check_freereg1_csv_file_for_correct_type,[:limit,:fix] => :environment do |t, args|
  ...
  Freereg1CsvFile.no_timeout.each do |my_file|
    ...
    file_ok = CheckFreereg1CsvFile.check_register_type(my_file,fix)
    if !file_ok[0]
    ...
    if processing == 100
      ...
      puts  "#{number_processed} processed at a rate of #{processing_time} ms/file"
      processing = 0
    end
  end
