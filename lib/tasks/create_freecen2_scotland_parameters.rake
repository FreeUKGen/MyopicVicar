  ...
  codes.each do |chapman|
    ...
    Freecen::CENSUS_YEARS_ARRAY.each do |year|
      ...
      FreecenPiece.where(year: year, chapman_code: chapman).order_by(piece_number: 1, parish_number: 1).no_timeout.each do |piece|
        ...
        if year == '1851' && chapman == 'AYR'
          # this code required because fc1 pices for all of 1851 are put in a single district of Ayr
          #so we make a temporary change to the piece to use either the 1841 district name of the sub place name
          piece_1841 = FreecenPiece.find_by(year: '1841', chapman_code: 'AYR', piece_number: piece.piece_number)
          if piece_1841.present?
            piece.district_name = piece_1841.district_name
          else
            district_name = piece.subplaces[0]['name']
            district_name = district_name.gsub(/&/, ' and ')
            piece.district_name = district_name
          end
        end


        if @district == piece.district_name
