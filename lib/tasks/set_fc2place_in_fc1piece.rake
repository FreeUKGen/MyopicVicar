...
task set_fc2place_in_fc1piece:  :environment do
  ...
  log_file.puts  "Country,FC1 Chapman Code,Year,FC1 Piece Number,Status,FC1 Place Name,Message"
  ...
  FreecenPiece.where(place_country:  "England").or(FreecenPiece.where(place_country:  "Wales")).or(FreecenPiece.where(place_country:  "Scotland")).order_by(chapman_code:1, year: 1, piece_number: 1).each do |fc1_piece|
    if fc1_piece.status == 'Online'
      ...
      unless message == ''
        ...
        log_file.puts  "#{fc1_piece.place_country},#{fc1_piece.chapman_code},#{fc1_piece.year},#{fc1_piece.piece_number},#{fc1_piece.status},#{place_name},#{out_message}"
      end
    end
  end
