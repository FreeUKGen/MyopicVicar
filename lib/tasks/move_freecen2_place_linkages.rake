namespace :freecen do
  ...
  task :move_freecen2_place_linkages, [:userid, :old, :new, :fix] => :environment do |_t, args|
    ...
    if old_place_record.blank? || new_place_record.blank?
      ...
      old_search_records_alt_pobs = SearchRecord.where(birth_county: old_place_record.chapman_code, birth_place: old_place_record.alternate_freecen2_place_names.alternate_name).count
      message_line = "Old Place Search Records Place Of Birth count = #{old_search_records_pobs}"
      output_to_log(message_file, message_line)
      message_line = "Old Place Search Records Place Of Birth as Alternate Place Name count = #{old_search_records_alt_pobs}"
      output_to_log(message_file, message_line)
      ...
      if old_search_records_alt_pobs.positive
        alternative_names = SortedSet.new
        alternative_names_list = ''
        alternate_pobs_used = SearchRecord.where(birth_county: old_place_record.chapman_code, birth_place: old_place_record.alternate_freecen2_place_names.alternate_name)
        alternate_pobs_used.each do |alternate_pob|
          alternative_names << alternate_pob.birth_place
        end
        ...
        message_line = "** Search Records Places Of Birth for old place used the following Alternate Place Names which should be added to the New Place if noy already present: [#{alternative_names_list}] **"
        output_to_log(message_file, message_line)
      end
      ...
      old_place_record.update_attributes(disabled: 'true', data_present: false, cen_data_years: []) if fixit
