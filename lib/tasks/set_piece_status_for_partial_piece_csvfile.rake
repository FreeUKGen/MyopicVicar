...
task :set_piece_status_for_partial_piece_csvfile, [:filename, :userid, :status]  => [:environment] do |t, args|
  ...
  if args[:filename].blank? or args[:userid].blank? or args[:status].blank?
    puts "ERROR: File name (xxx.csv), Userid (neither is case sensitive) and Status (Online or Part) must be provided as arguments"
  else
    if args[:status] != "Online" && args[:status] != "Part"
      ...
      csvfile = FreecenCsvFile.where(:file_name.downcase => file_name_dc, :userid_lower_case => userid_dc).first
      if csvfile.blank?
