class DeleteEntriesRecordsForRemovedBatches
  def self.process(len,fr)
    ...
    p "There are #{userids} userids and #{total_userid_files.length} userid files" 
    ...
    total_base_files_hash_copy.each_pair do |user,file_array|
        unless file_array.empty?
          @@message_file.puts user
          @@message_file.puts file_array
          file_array.each do |my_file|
            delete_file = File.join(base_directory,user,my_file)
            @@message_file.puts "Deleting #{user} #{delete_file}"
            Freereg1CsvFile.where(userid: user,file_name: my_file).all.each do |del_file|
              #del_file.destroy
