class MoveCivilParishesToAnotherCounty
  ...
  def self.process(limit, file_name)
    ...
    file_for_warning_messages = 'log/move_civil_parishes_to_another_county.txt'
    ...
    array.each do |move|
      ...
      while parishes < number_of_civil_parishes
        ...
        message_file.puts "Moving #{original_parish} from #{piece.number} in #{original_district.name} in #{new_chapman_code} for #{year}"
        puts "Moving #{original_parish} from #{piece.number} in #{original_district.name} in #{new_chapman_code} for #{year}"
