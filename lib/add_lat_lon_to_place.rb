...
require "#{Rails.root}/app/models/place"
 ...
 def self.process(type)
        ...
        #master has a loaction for this place
          if place.location.nil? || (place.location[0].nil? && place.location[1].nil?)
            ...
            if (place.location[0] == location.latitude) || (place.location[1] == location.longitude) 
             #they are the same
            p "\" #{place.place_name}\", #{place.chapman_code} is up to date"
            else
            #they are different
             place.location[0] = location.latitude
             place.location[1] == location.longitude
                p "\" #{place.place_name}\", #{place.chapman_code} is updated" 
   ...
   puts "#{number_added} places added #{number_updated} updated and #{number_not_found} not found"
 end #end method
end