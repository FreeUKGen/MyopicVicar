module FreecenValidations
  ...
  def FreecenValidations.valid_piece?(field)
  ...
  def FreecenValidations.cleannumeric(field)
    #not convinced this code is effective or needed
    return true if field.nil? || field.empty?
  ...
  def FreecenValidations.cleanage(field)

    #the planning team requested this code be deactivated for burials
    return true if field.nil? || field.empty?
    return true if field =~ VALID_UCF
    return true if field =~ WILD_CHARACTER
    return false unless FreecenValidations.cleannumeric(field)
  ...
  def  FreecenValidations.cleansex(field)
    case
    when field.nil?
      return true
  ...
  def FreecenValidations.cleancondition(field)
    return true if field.nil? || field.empty?
  ...
  def FreecenValidations.birth_date_less_than_baptism_date(birth,baptism)
  ...
  def FreecenValidations.cleandate(field)
    ...
    when a.length == 3
      ...
      check = FreecenValidations.check_year(a[2])
      return check
    ...
    when a.length == 2
      ...
      check = FreecenValidations.check_year(a[1])
      return check
    when a.length == 1
      #deal with dates that are year only
      check = FreecenValidations.check_year(a[0])
      return check
    else
  ...
  def FreecenValidations.year_extract(field)
    if field.blank?
      year = nil
      return
    else
  ...
  def FreecenValidations.modern_date_valid?(field)
  ...
  def FreecenValidations.valid_chapman_code?(field)
  ...
  def FreecenValidations.valid_place?(field,chapman)
  ...
  def FreecenValidations.valid_church?(church_name,chapman_code,place_name)
