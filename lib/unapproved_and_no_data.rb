class UnapprovedAndNoData
  ...
  def self.process(limit)
    ...
    limit = limit.to_i
    puts "checking places for unapproved and no data present"
    message_file.puts "Place Name,Chapman Code,County,Original Country,Country,Original Country"
    record_number = 0
    number = Place.count
     Place.all.each do |my_entry|
      if my_entry.error_flag == "Place name is not approved" && my_entry.data_present == 'false' 
     ...
     puts "There were #{record_number} unapproved places with no data in #{number} places"
  end
end
