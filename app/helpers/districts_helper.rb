module DistrictsHelper
  def show_page_back_link(link_to_hash = {})
    ...
    when link_hash[:district].present? && !link_hash[:search_query].present?
      a= link_to 'Back to overview', "/districts/districts_overview", class: " btn   btn--small"
    else
      a = link_to 'Back to overview', "/districts/districts_overview", class: "btn   btn--small"
    end
    return a
  ...
  def show_back_to_district_link(link_to_hash = {})
    ...
    when link_hash[:district].present?
      a = link_to 'Back to district', districts_select_district_path(id: link_hash[:district].id), class: "btn   btn--small"
    end
    return a
  end
end