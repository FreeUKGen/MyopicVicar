module FreecenCsvFilesHelper
  ...
  def edit_headers
    link_to 'Edit headers', edit_freecen_csv_file_path(@freecen_csv_file), method: :get, class: 'btn   btn--small'
  end

  def download_batch
    link_to 'Download batch', download_freecen_csv_file_path(@freecen_csv_file),
  ...
  def list_zero_year_entries
    link_to 'Listing of zero year entries', zero_year_freecen_csv_file_path(@freecen_csv_file), method: :get, class: 'btn   btn--small'
  end

  def list_embargoed_entries
    link_to 'Listing of embargoed entries', embargoed_entries_freecen_csv_file_path(@freecen_csv_file), method: :get, class: 'btn   btn--small'
  end

  def unique_names
    link_to 'Unique names', unique_names_freecen_csv_file_path(object: @freecen_csv_file.id), method: :get, class: 'btn   btn--small'
  end

  def remove_batch
    link_to 'Remove batch', remove_freecen_csv_file_path(@freecen_csv_file),  data: { confirm: 'Are you sure you want to remove this batch' }, class: 'btn   btn--small', method: :get
  end

  def replace_batch
    link_to 'Replace batch', edit_csvfile_path(@freecen_csv_file), method: :get,
      data: { confirm:  'Are you sure you want to replace these records?' }, class: 'btn   btn--small'
  end

  def relocate_batch
    link_to 'Relocate batch', relocate_freecen_csv_file_path(@freecen_csv_file),
  ...
  def reprocess_batch
    link_to '(Re)Process batch', reprocess_physical_file_path(@freecen_csv_file), class: 'btn   btn--small', method: :get,
      data: { confirm:  'Are you sure you want to reprocess this file?' }
  end

  def delete_file
    link_to 'Delete original file and all associated batch entries', freecen_csv_file_path(@freecen_csv_file),
  ...
  def which_file_gaps_link(gaps)
    file = @freecen_csv_file.id
    if gaps
      link_to 'List Gaps', gaps_path(register: @register, freecen_csv_file: file), method: :get, class: 'btn   btn--small'
    elsif @freecen_csv_file.register_type == 'PR'
      link_to 'Create Gap', new_gap_path(register: @register, freecen_csv_file: file), method: :get, class: 'btn  btn--small'
    end
  end
  ...
  def file_format_ucf_list(record)
    ...
    entry = search.freecen_csv_entry if search.present?
    if entry.present?
      link_to "#{entry.id.to_s}", freecen_csv_entry_path(entry.id.to_s, from: file), method: :get
    end
  end
  ...
  def mindata(file)
    ...
    if zero_entry || file.datemin == '0'
      link_to "#{file.datemin}", zero_year_freecen_csv_file_path(id: file.id), method: :get, class: "btn btn--small"
    else
      "#{file.datemin}"
    end
  end
  ...
  def zero_entry(file)
    zero_entry = file.zero_entries
    if zero_entry || file.datemin == '0'
      link_to 'Yes', zero_year_freecen_csv_file_path(id: file.id), method: :get, class: "btn btn--small"
    else
      'No'
    end
  end
