module Freecen2CountyContentsHelper
  def county_contents_check_for_partials(piece_ids, pieces_online)
    partial_flag = ''
  ...
  def county_contents_show_percentage(pieces_online, pieces)
    percent = 0
    percent = ((pieces_online.to_f / pieces.to_f) * 100).round(1) if pieces_online.positive?
    content_tag(:td, percent.to_s)
  end

  def county_county_index_link(county_description, place_description)
    if place_description == 'all'
      return link_to "Back to Records for #{county_description}", index_by_county_freecen2_county_contents_path(county_description: county_description), method: :get, :class => 'btn btn--small'
    else
      return link_to "Choose another Place in #{county_description}", index_by_county_freecen2_county_contents_path(county_description: county_description), method: :get, :class => 'btn btn--small'
    end
  end

  def county_names_exist_for_place_year(place_id, year)
    has_names = false
  ...
  def county_locate_place_link(place_id, button_size)
  ...
  def county_records_for_piece(piece_id, piece_status)
    record_count = 0
