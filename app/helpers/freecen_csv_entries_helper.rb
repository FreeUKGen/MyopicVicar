module FreecenCsvEntriesHelper
  ...
  def previous_list_entry
    link_to "Previous #{Freecen::Listings::NAMES[@type]} entry", freecen_csv_entry_path(@previous_list_entry, type: @type), method: :get, class: "btn btn--small",
      title: 'Show the previous entry in the View List' if @previous_list_entry.present?
  end

  def next_list_entry
    link_to "Next #{Freecen::Listings::NAMES[@type]} entry", freecen_csv_entry_path(@next_list_entry, type: @type), method: :get, class: "btn btn--small",
      title: 'Show the previous entry in the View List' if @next_list_entry.present?
  end
  ...
  def reject_entry
    if @freecen_csv_entry.file_validation && @freecen_csv_entry.record_valid == 'true'
      ...
      link_to 'Remove Acceptance', revalidate_freecen_csv_entry_path(@freecen_csv_entry), method: :get, class: "btn btn--small",
