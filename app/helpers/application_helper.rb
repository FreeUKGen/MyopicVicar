...
module ApplicationHelper
  ...
  def nav_about_page_link
    link_to( 'About', '/help/about_new', class: check_current_page('help/about_new'))
  end
  ...
  def footer_records_stats(stat, type)
    return 0 unless stat.respond_to?(:where)

    rec = stat.where(record_type: type).first
    return 0 if rec.blank? || !rec.respond_to?(:total_records)

    rec.total_records.to_i
  end
  ...
  def my_heritage_720_90_first_bmd
    banner = <<-HTML
    <ins class='dcmads' style='display:inline-block;width:728px;height:90px'
      data-dcm-placement='N217801.4536833FREEBMD/B28102710.433480851'
      data-dcm-rendering-mode='iframe'
      data-dcm-https-only
