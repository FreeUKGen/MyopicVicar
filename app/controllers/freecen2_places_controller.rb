...
class Freecen2PlacesController < ApplicationController
  ...
  def review_move
    ...
    if params[:review_move_fc2_place][:mode] == 'Update'
      pid1 = RakeSpawn.run("freecen:move_freecen2_place_linkages[#{userid},#{@place_from_rec.id},#{@place_to_rec.id},Y]")
    else
      pid1 = RakeSpawn.run("freecen:move_freecen2_place_linkages[#{userid},#{@place_from_rec.id},#{@place_to_rec.id},N]")
    end
