...
class ManageSyndicatesController < ApplicationController
  ...
  def syndicates_for_selection
    all = true if %w[volunteer_coordinator data_manager system_administrator documentation_coordinator SNDManager CENManager executive_director project_manager].include?(@user.person_role)
