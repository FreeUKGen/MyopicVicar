...
class FreecenCsvFile
  ...
  validates_inclusion_of :record_type, in: RecordType::ALL_FREECEN_TYPES + [nil]
  #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  class << self
    ...
    def valid_freereg1_csv_file?(freereg1_csv_file)
      ...
      freereg1_csv_file_object = FreecenCsvFile.find(freereg1_csv_file)
  ...
  def calculate_distribution
    ...
    datemin, zero_entries = FreecenCsvFile.calculate_min_year(entries)
    datemax = FreecenCsvFile.calculate_max_year(entries)
    number_of_records, daterange = FreecenCsvFile.calculate_range_distribution(entries)
  ...
  def check_file
    ...
    success[0] = true
    success[1] = ""
    FreecenCsvFile.file_name(self.file_name).userid(self.userid).hint("file_name_1_userid_1_county_1_place_1_church_name_1_register_type_1").each do |batch|
      case
      when batch.nil?
        success[0] = false
  ...
  def force_unlock
    batches = FreecenCsvFile.where(:file_name => self.file_name, :userid => self.userid).all
  ...
  def promulgate_userid_change(new_userid,old_userid)
    ...
    FreecenCsvFile.userid(old_userid).file_name(self.file_name).each do |batch|
  ...
  def get_unique_names
    entries = Hash.new
    all_entries = FreecenCsvEntry.where(:freereg1_csv_file_id => self.id)
  ...
  def zero_year_entries
    ...
    FreecenCsvEntry.freereg1_csv_file(id).year(nil).each do |entry|
