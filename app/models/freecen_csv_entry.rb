  ...
  class << self
      ...
      if flexible
        ...
        if record[:data_transition] == 'Civil Parish'
          ...
          unless success
            ...
            message = message + "Error: line #{num} Civil Parish has too many characters #{record[:civil_parish]}\r\n " if messagea == 'field length'
            message = message + "Error: line #{num} Civil Parish is blank #{record[:civil_parish]}\r\n" if messagea == 'blank'
            message = message + "Error: line #{num} Civil Parish has invalid text #{record[:civil_parish]}\r\n" if messagea == 'VALID_TEXT'
          end
        end
        if ['Enumeration District', 'Civil Parish'].include?(record[:data_transition])
          ...
          unless success
            overall_success = false
            message = message + "Error: line #{num} Enumeration District #{record[:civil_parish]} is #{messagea}\r\n"
          end
        end
        ...
        if ['Enumeration District', 'Civil Parish', 'Folio', 'Page'].include?(record[:data_transition])
          ...
          unless success
            overall_success = false
            message = message + "Error: line #{num} Page number #{record[:page_number]} is #{messagea}\r\n"
          end
        end
    ...
    def validate_dwelling(record, num, flexible)
      ...
      if flexible
        ...
        message = ''
        success, messagea = FreecenValidations.fixed_schedule_number?(record[:schedule_number], record[:data_transition])
        unless success
          overall_success = false
          message = message + "Error: line #{num} Schedule number #{record[:schedule_number]} is #{messagea}\r\n"
        end
        success, messagea = FreecenValidations.fixed_house_number?(record[:house_number], record[:data_transition])
        unless success
          overall_success = false
          message = message + "Error: line #{num} House number #{record[:house_number]} is #{messagea}\r\n"
        end
        success, messagea = FreecenValidations.fixed_house_address?(record[:house_or_street_name], record[:data_transition])
        unless success
          overall_success = false
          if messagea == '?'

            message = message + "Error: line #{num} House address #{record[:house_or_street_name]}  has trailing ?. Removed and flag set"
            record[:uncertainy_location] = 'x' unless ['b', 'n', 'u', 'v'].include?(record[:uncertainy_location])
        ...
        success, messagea = FreecenValidations.fixed_uncertainy_location?(record[:uncertainy_location])
        unless success
          overall_success = false
          message = message + "Error: line #{num} Special use #{record[:uncertainy_location]} is #{messagea}\r\n"
        end
      end
      [overall_success, message]
    end
    ...
    def validate_individual(record, num, flexible)
      ...
      if flexible
        ...
        unless success
          overall_success = false
          if messagea == '?'
            message = message + "Error: line #{num} Surname  #{record[:surname]} has trailing ?. Removed and flag set"
        ...
        unless success
          overall_success = false
          if messagea == '?'
            message = message + "Error: line #{num} Forenames  #{record[:forenames]} has trailing ?. Removed and flag set"
            record[:uncertainty_name] = 'x'
        ...
        unless success
          overall_success = false
          if messagea == '?'
            message = message + "Error: line #{num} Occupation #{record[:occupation]} has trailing ?. Removed and flag set"
