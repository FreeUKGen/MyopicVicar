...
class Freereg1CsvFile
  ...
  def combine_now_new(holding,individual,index,number)
 
    if index == 0
      holding['transcriber_name'] = Array.new
      holding['credit_name'] = Array.new
      holding['records'] = individual.records
      holding['datemax'] = individual.datemax.to_i
      holding['datemin'] = individual.datemin.to_i
      holding['datemin'] = Freereg1CsvFile.calculate_min_year(individual.freereg1_csv_entries) if holding['datemin'] < FreeregValidations::YEAR_MIN.to_i
