...
class BestGuess < FreebmdDbBase
  ...
  def self.create_csv_file(start_quarter, end_quarter, district_number, record_count, skip_count)
    ...
    n = 0
    district.records.where(QuarterNumber: start_quarter.to_i..end_quarter.to_i).order(:QuarterNumber).offset(skip_count.to_i).select(:Surname, :GivenName, :AgeAtDeath, :DistrictNumber, :DistrictFlag, :District, :Volume, :Page, :QuarterNumber, :RecordNumber, :CountyComboID, :RecordTypeID).find_in_batches(batch_size: record_count.to_i) do |record_batch|
      n += 1
