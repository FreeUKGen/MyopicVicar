class Assignment
  ...
  class << self
    ...
    def filter_assignments_by_userid(user_id,syndicate,image_server_group_id)
      ...
      i_ids = ImageServerImage.where(:assignment_id=>{'$in'=>a_ids.map(&:first)}).order_by(image_file_name: 1).pluck(:id, :assignment_id, :image_server_group_id, :image_file_name, :status, :difficulty, :notes)
      (assignment_id, image_assignment_id, image_group_id, image, group_name) = prepare_for_parsing(a_ids,i_ids)
