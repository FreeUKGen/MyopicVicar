class Source
  ...
  def errors_in_fields
    ...
    errors.add(:start_date, 'Invalid start year') if start_date.present? && (start_date.to_i <= 1 || start_date.to_i > Year.now)
    errors.add(:end_date, 'Invalid end year') if end_date.present? && (end_date.to_i <= 1 || end_date.to_i > Year.now)
