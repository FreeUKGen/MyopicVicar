class UseridDetail
  ...
  def userid_and_email_address_does_not_exist
    ...
    errors.add(:userid, "User Already exists") if User.where(:username => self[:userid]).exists?
    errors.add(:email_address, "Userid email already exists") if UseridDetail.where(:email_address => self[:email_address]).exists?
    errors.add(:email_address, "User email already exists") if User.where(:email => self[:email_address]).exists?
  end
  ...
  def check_exists_in_refinery
    auth_user = User.where(:username => self.userid).first
    if auth_user.nil?
      return[false,"There is no user entry"]
    else
      return[true]
    end
  end
  ...
  def delete_refinery_user_and_userid_folder
    auth_user = User.where(:username => self.userid).first
    auth_user.destroy unless auth_user.nil?
  ...
  def email_address_does_not_exist
    ...
    if other.present? && (auth_user.nil? || other.id != auth_user.id)
      errors.add(:email_address, "User email already exists on change")
    end
  end
