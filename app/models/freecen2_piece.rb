class Freecen2Piece
  ...
  field :piece_name, type: String
  validates :piece_name, presence: true
  field :piece_tnaid, type: String
  validates :piece_tnaid, presence: true
  field :piece_number, type: String
  validates :piece_number, presence: true
  field :piece_code, type: String
  field :piece_notes, type: String
  ...
  delegate :year, :chapman_code, :name, :tnaid, :code, :note, to: :freecen2_district, prefix: 'district', allow_nil: true

  embeds_many :freecen2_civil_parishes
  ...
  class << self
    ...
    def county_year_totals(chapman_code)
      ...
      Freecen::CENSUS_YEARS_ARRAY.each do |year|
        ...
        totals_individuals[year] = Freecen2Piece.chapman_code(chapman_code).status('Online').year(year).sum(:num_individuals)
        totals_pieces[year] = Freecen2Piece.chapman_code(chapman_code).year(year).count
        totals_pieces_online[year] = Freecen2Piece.chapman_code(chapman_code).status('Online').year(year).length
        Freecen2Piece.chapman_code(chapman_code).status('Online').year(year).each do |piece|
    ...
    def check_piece_params(piece_params, controller_name)
      error_list = []
      piece = Freecen2Piece.find_by(year: piece_params[:year], chapman_code: piece_params[:chapman_code], piece_number: piece_params[:piece_number])
